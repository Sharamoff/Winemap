...
function SendMail() {
  if(document.getElementById("customer_email").value.length < 3) {
    window.location.href = "https://gvinico.com/account/login?checkout_url=/pages/wishlist";
  }
  else {
    var params = {
    wish_list : ("" + getWishlist()).replace(/,/g, ', ').replaceAll('39qvevri','39 Qvevri:').replaceAll('pongino-wine','Pongino Wine:').replaceAll('shatiri','Shatiri:').replaceAll('anbani','Anbani Wine:').replaceAll('velisi','Chateau Velisi:').replaceAll('leladze','David Leladze Family Winery:').replaceAll('levanismarani','Levani\'s Marani Akhmeta:').replaceAll('-',' '),
    customer_email : document.getElementById("customer_email").value.replace(/\//g,'')
    }
    emailjs.send("service_tvy55yp", "template_oi8fwgw", params).then(function(response) {
...
var dialog = document.querySelector('dialog');
   document.querySelector('#wishlist_clear').onclick = function() {
    dialog.showModal();
   }
document.querySelector('#clear_and_close_dialog').onclick = function() {
    resetWishlist();
    dialog.close();
   }
